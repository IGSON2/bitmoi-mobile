    ...
    return (
        <div className="history">
            <div className="history_summary">
                ...
                <div className="history_row_box">
                    <div>전체 수익금</div>
                    <div>{`${(Math.floor(100000*summary.total_pnl)/100000).toLocaleString("en-US",{maximumFractionDigits:0})} USDT`}</div>
                </div>
                <div className="history_row_box">
                    <div>이번 달 수익금</div>
                    <div>{`${(Math.floor(100000*summary.month_pnl)/100000).toLocaleString("en-US",{maximumFractionDigits:0})} USDT`}</div>
                </div>
            </div>
            {
                        ...
                        <div className="history_score" key={index}>
                            <div className="history_row_box">
                                <div className="history_score_position_lev">
                                    <div className={`history_score_position ${score.position==="LONG"?"position_long":"position_short"}`}>{score.position}</div>
                                    <div className="history_score_leverage">X{score.leverage}</div>
                                </div>
                                <div className="history_score_created_at">{Timeformatter(score.created_at,false)}</div>
                            </div>
                            <div className="history_row_box">
...
function calcSummary(scores: Score[]){
    let summary:Summary ={
        total: 0,
        total_win: 0,
        total_lose: 0,
